					 ...
					 const EVP_MD *md, const EVP_MD *mgf1md)
{
	...
	if (beta_PKCS1_MGF1(dbmask, emlen - mdlen, seed, mdlen, mgf1md) < 0)
		return 0;
	for (i = 0; i < emlen - mdlen; i++)
		db[i] ^= dbmask[i];

	if (beta_PKCS1_MGF1(seedmask, mdlen, db, emlen - mdlen, mgf1md) < 0)
		return 0;
	for (i = 0; i < mdlen; i++)
		seed[i] ^= seedmask[i];
					   ...
					   const EVP_MD *md, const EVP_MD *mgf1md)
{
	...
	if (beta_PKCS1_MGF1(seed, mdlen, maskeddb, dblen, mgf1md))
		return -1;
	for (i = 0; i < mdlen; i++)
		seed[i] ^= padded_from[i];

	if (beta_PKCS1_MGF1(db, dblen, seed, mdlen, mgf1md))
		return -1;
	for (i = 0; i < dblen; i++)
		db[i] ^= maskeddb[i];
